...
* PrimaveraPMFileWriter.write fails with java.lang.IllegalArgumentException
* Microsoft Project 2016 : Need to set 'Stop' and 'Resume'  properties for net.sf.mpxj.ResourceAssignment
...
* NullPointerException in CriteriaReader.getConstantValue
* Allow a character encoding to be specified when reading an XER file
...
* synchronizeTaskIDToHierarchy clears list of tasks
...
* Incorrect resource types are read (contributed by Colin Rodriguez)
...
* Not all project calendars are read in for Project 2013 files (based on a contribution by Colin Rodriguez)
...
## 4.7.4 (25/02/2015)
* Failed to read project containing CodePage 1250 text.
* MS Project 2010: tasks with null baseline dates
...
* Defensive changes to avoid exceptions when reading MPP files
* Deleted tasks being read from mpp file
...
* All resources have "Material" property
* Updated Primavera PM XML file reader to capture the Project ID to align with data read from XER file/database (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to ensure task percent complete supports Physical Percent, Duration Percent and Units Percent (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to ensure task baseline values match values read from XER file/database (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to ensure task actual duration to matches value read from XER file/database (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to read the task duration (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to read task LateStart, LateFinish, EarlyStart, EarlyFinish attributes correctly (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to read task Start and End correctly (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to identify milestones (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to set the task Critical attribute (contributed by Nathaniel Marrin)
* Updated Primavera PM XML file reader to include costs (contributed by Nathaniel Marrin)
* Updated Primavera XER/Database readers to read task Start and End correctly (contributed by Nathaniel Marrin)
...
* Unable to load mpp from project-2013
* Primavera currency files without currency information
* PrimaveraReader cannot handle files with more than 30 user defined fields
* setMilestone() issue
* MPXJ 4.4 and 2013 files - invalid load of task data
...
* ResourceAssignment.getTaskUniqueID() returns null.
* Wrong project name in MPX file.
* Wrong title in XML file when importing from XER file.
* Wrong record number for resource calendar in MPX file.
* In the XML file the element field SaveVersion is missing.
* Loop when import task with 0% on units of works in resources.
* French locale NA incorrect.
* Invalid dependency between child and parent.
* Missing tasks from MS Project 2010 mpp file.
* Wrong WBS code and WBS when converting a Primavera XER file.
* Error reading XER file with German localisation for numbers.
* TimephasedResourceAssignments with negative TotalWork.
* Wrong currency symbol in the exported file.
* TimephasedResourceAssignment end date not correct.
* Calendar hours are incorrect.
* NullReferenceException with getTimephasedBaselineWork.
* Outline number is null when opening Project 2003 MPP file.
* Unable to parse note (unknown locale).
* MPP9Reader marks all tasks after a null task as null.
...
* Error converting Mpp to planner.
* MSPDI Linklag for TimeUnit.Percent.
* Error reading calendars for 2010 files.
* Duplication of calendar id.
* Wrong task start.
* Wrong start and finish dates for 2010 files.
...
* MPXJ API returns the incorrect start date of a manual task.
* Task id incorrect after importing from MPP14 file.
* MPXJ 4.0 fails to work with Project 2010 format.
* Index was outside the bounds of the array.
* header.getHonorConstraints() is not working in case of MPP.
* Empty notes appear for all tasks when saving in XML format.
* All Extended Attributes always added when using MSPDIWriter.
* Baseline/Actual Work in 2010 MPP missing.
* ResourceAssignment getCalendar not using IgnoreResourceCalendar flag
* ExternalTaskProject value missing.
* Deleted Primavera tasks handling problem.
* Latest CVS version gives wrong values for inactive field.
* Task ID order when creating a project file is not correct.
...
* POI License in legal folder of download wrong.
...
* getRecurring() task is not working
* getStandardRateFormat() is returning 'null'
* getWeekStartDay() is not working.
* getDaysPerMonth() is not working.
* Resource.getNotes() not working for MPP12 file.
...
## 3.1.0 (20/05/2009)
* Plan file fails to load.
* Resource Assignment Normaliser rounding problem.
* Column alignment values are incorrect.
* NullPointerException in parseExtendedAttribute() (Contributed by Paul Pogonyshev).
* .0 at the end of WBS code and outline number (Contributed by Paul Pogonyshev).
* Too strict net.sf.mpxj.mpd.ResultSetRow.
* Generated planner file can't be opened.
* Support for loading global.mpt.
* Lowercase table name won't work with db on linux machines.
* Standard Calendar localization import problem.
* Strange duration conversion from database
* FilterCriteria not being read in properly (Contributed by James Styles)
...
* Task getNumber*() methods return inaccurate large values.
* Duplicate task in file.getChildTasks() when opening MPX.
* Relation.getTask returns null.
* Task.getSplits() not consistent.
* WBS Field not imported Mpp12.
* There are some conflict in TaskField.
* MSPDIReader is not setting calendarName in projectHeader.
* Write resource calendar with exceptions only.
* File loses predecessors.
* Resources not bring read.
...
* Import from Project 2007 ignores some tasks.
* Crash on priority not set in MSPDI-file.
* Resource start/finish dates with MSP 2007.
* MS Project 2007: Calendar exceptions dates are wrong.
