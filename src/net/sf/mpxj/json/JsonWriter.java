...
/**
 * This class creates a new JSON file from the contents of a ProjectFile
 * instance.
 */
public final class JsonWriter extends AbstractProjectWriter {
  ...
  /**
   * Write attributes for an individual custom field. Note that at present we are
   * only writing a subset of the available data... in this instance the field
   * alias. If the field does not have an alias we won't write an entry.
   *
  ...
  private void writeCalendars() throws IOException {
    ...
    for (ProjectCalendar calendar : m_projectFile.getCalendars()) {
      ...
      m_writer.writeNameValuePair("base_calendar_name",
          calendar.getParent() == null ? "" : calendar.getParent().getName());
  ...
  /**
   * This method writes task data to a JSON file. Note that we write the task
   * hierarchy in order to make rebuilding the hierarchy easier.
   */
  ...
  /**
   * This method is called recursively to write a task and its child tasks to the
   * JSON file.
   *
