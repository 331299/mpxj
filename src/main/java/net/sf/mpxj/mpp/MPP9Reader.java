 ...
 * implementation allows the file to be read, and the data it contains exported
 * as a set of MPX objects. These objects can be interrogated to retrieve any
 * required data, or stored as an MPX file.
 */
final class MPP9Reader implements MPPVariantReader {
   /**
    * This method is used to process an MPP9 file. This is the file format used by
    * Project 2000, 2002, and 2003.
    *
    ...
    * @param root   Root of the POI file system.
    */
   private void populateMemberData(MPPReader reader, ProjectFile file, DirectoryEntry root)
      ...
      // String readPassword =
      // MPPUtility.decodePassword(props9.getByteArray(Props.PROTECTION_PASSWORD_HASH),
      // m_inputStreamFactory.getEncryptionCode());
      // // It looks like it is possible for a project file to have the password
      // protection flag on without a password. In
      // // this case MS Project treats the file as NOT protected. We need to do the
      // same. It is worth noting that MS Project does
      // // correct the problem if the file is re-saved (at least it did for me).
      // if (readPassword != null && readPassword.length() > 0)
      // {
      // // See if the correct read password was given
      // if (reader.getReadPassword() == null ||
      // reader.getReadPassword().matches(readPassword) == false)
      // {
      ...
      VarMeta outlineCodeVarMeta = new VarMeta9(
            new DocumentInputStream(((DocumentEntry) outlineCodeDir.getEntry("VarMeta"))));
      m_outlineCodeVarData = new Var2Data(outlineCodeVarMeta,
            new DocumentInputStream(((DocumentEntry) outlineCodeDir.getEntry("Var2Data"))));
      m_projectProps = new Props9(m_inputStreamFactory.getInstance(m_projectDir, "Props"));
      // MPPUtility.fileDump("c:\\temp\\props.txt",
      // m_projectProps.toString().getBytes());
   ...
   /**
    * Read sub project data from the file, and add it to a hash map indexed by task
    * ID.
    *
   ...
   private void processSubProjectData() {
      ...
      if (subProjData != null) {
         ...
         while (offset < itemCountOffset) {
            ...
            // System.out.println ("type=" + ByteArrayHelper.hexdump(itemHeader, 16, 1,
            // false));
    ...
    * @param subprojectIndex index of the subproject, used to calculate unique id
    *                        offset
   ...
   /**
    * This method process the data held in the props file specific to the visual
    * appearance of the project data.
    */
   ...
   /**
    * Create an index of base font numbers and their associated base font
    * instances.
    * 
   ...
   private void processCustomValueLists() throws IOException {
      CustomFieldValueReader9 reader = new CustomFieldValueReader9(m_projectDir, m_file.getProjectProperties(),
            m_projectProps, m_file.getCustomFields());
      reader.process();
   }

   /**
    * Retrieves the description value list associated with a custom task field.
    * This method will return null if no descriptions for the value list has been
   ...
   public List<Object> getTaskFieldValues(ProjectProperties properties, FieldType field, byte[] data) {
      ...
      case DURATION:
         while (offset + 6 <= data.length) {
            Duration duration = MPPUtility.getAdjustedDuration(properties, MPPUtility.getInt(data, offset),
                  MPPUtility.getDurationTimeUnits(MPPUtility.getShort(data, offset + 4)));
            list.add(duration);
            offset += 6;
         }
         break;
   ...
   /**
    * This method maps the task unique identifiers to their index number within the
    * FixedData block.
    *
      ...
      for (int loop = 3; loop < itemCount; loop++) {
         ...
         if (data != null) {
            ...
            if ((flags & 0x02) != 0) {
               ...
               //
               uniqueID = MPPUtility.getShort(data, TASK_UNIQUE_ID_FIXED_OFFSET); // Only a short stored for deleted
                                                                                  // tasks?
               ...
               if (data.length == NULL_TASK_BLOCK_SIZE) {
                  ...
                  if (maxSize == 0 || ((data.length * 100) / maxSize) > 75) {
                     ...
                     key = Integer.valueOf(uniqueID);
                     // Accept this task if it does not have a deleted unique ID or it has a deleted
                     // unique ID but the name is not null
   ...
   /**
    * This method maps the resource unique identifiers to their index number within
    * the FixedData block.
    *
    * @param fieldMap     field map
    ...
    */
   private TreeMap<Integer, Integer> createResourceMap(FieldMap fieldMap, FixedMeta rscFixedMeta,
         FixedData rscFixedData) {
   ...
   /**
    * The format of the calendar data is a 4 byte header followed by 7x 60 byte
    * blocks, one for each day of the week. Optionally following this is a set of
    * 64 byte blocks representing exceptions to the calendar.
    */
   ...
   /**
    * This method extracts and collates task data. The code below goes through the
    * modifier methods of the Task class in alphabetical order extracting the data
    * from the MPP file. Where there is no mapping (e.g. the field is calculated on
    * the fly, or we can't find it in the data) the line is commented out.
    *
    * The missing boolean attributes are probably represented in the Props section
    * of the task data, which we have yet to decode.
    *
    * @throws IOException
    */
   private void processTaskData() throws IOException {
      ...
      Var2Data taskVarData = new Var2Data(taskVarMeta,
            new DocumentInputStream(((DocumentEntry) taskDir.getEntry("Var2Data"))));
      FixedMeta taskFixedMeta = new FixedMeta(new DocumentInputStream(((DocumentEntry) taskDir.getEntry("FixedMeta"))),
            47);
      FixedData taskFixedData = new FixedData(taskFixedMeta, m_inputStreamFactory.getInstance(taskDir, "FixedData"),
            768, fieldMap.getMaxFixedDataSize(0));
      ...
      // The var data may not contain all the tasks as tasks with no var data assigned
      // will
      // not be saved in there. Most notably these are tasks with no name. So use the
      // task map
      ...
      for (int loop = 0; loop < uniqueIdArray.length; loop++) {
         ...
         if (temp != null) {
            // Task with this id already exists... determine if this is the 'real' task by
            // seeing
            // if this task has some var data. This is sort of hokey, but it's the best
            // method i have
            // been able to see.
            if (!taskVarMeta.getUniqueIdentifierSet().contains(uniqueID)) {
               // Sometimes Project contains phantom tasks that coexist on the same id as a
               // valid
               // task. In this case don't want to include the phantom task. Seems to be a very
               // rare case.
               continue;
            } else if (temp.getName() == null) {
               // Ok, this looks valid. Remove the previous instance since it is most likely
               // not a valid task.
         ...
         task.setEstimated(getDurationEstimated(
               MPPUtility.getShort(data, fieldMap.getFixedDataOffset(TaskField.ACTUAL_DURATION_UNITS))));
         ...
         processHyperlinkData(task,
               taskVarData.getByteArray(uniqueID, fieldMap.getVarDataKey(TaskField.HYPERLINK_DATA)));
         ...
         task.setOutlineCode(1, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE1_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(2, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE2_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(3, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE3_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(4, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE4_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(5, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE5_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(6, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE6_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(7, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE7_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(8, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE8_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(9, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE9_INDEX), OUTLINECODE_DATA));
         task.setOutlineCode(10, m_outlineCodeVarData
               .getUnicodeString((Integer) task.getCachedValue(TaskField.OUTLINE_CODE10_INDEX), OUTLINECODE_DATA));
         ...
         if (task.getName() == null
               && ((task.getStart() == null || task.getStart().getTime() == MPPUtility.getEpochDate().getTime())
                     ...
                     || (task.getCreateDate() == null
                           || task.getCreateDate().getTime() == MPPUtility.getEpochDate().getTime()))) {
   ...
   /**
    * The project files to which external tasks relate appear not to be held
    * against each task, instead there appears to be the concept of the "current"
    * external task file, i.e. the last one used. This method iterates through the
    * list of tasks marked as external and attempts to ensure that the correct
    * external project data (in the form of a SubProject object) is linked to the
    * task.
    *
   ...
   private void processExternalTasks(List<Task> externalTasks) {
      ...
      for (Task currentTask : externalTasks) {
         ...
         if (currentSubProject != null) {
            // System.out.println ("Task: " +currentTask.getUniqueID() + " " +
            // currentTask.getName() + " File=" + currentSubProject.getFullPath() + " ID=" +
            // currentTask.getExternalTaskID());
   ...
   /**
    * This method is used to extract the task hyperlink attributes from a block of
    * data and call the appropriate modifier methods to configure the specified
    * task object.
    *
    * @param task task instance
   ...
   /**
    * This method is used to extract the resource hyperlink attributes from a block
    * of data and call the appropriate modifier methods to configure the specified
    * task object.
    *
    * @param resource resource instance
    * @param data     hyperlink data block
   ...
   private void processResourceData() throws IOException {
      ...
      Var2Data rscVarData = new Var2Data(rscVarMeta,
            new DocumentInputStream(((DocumentEntry) rscDir.getEntry("Var2Data"))));
      FixedMeta rscFixedMeta = new FixedMeta(new DocumentInputStream(((DocumentEntry) rscDir.getEntry("FixedMeta"))),
            37);
      ...
      for (int loop = 0; loop < uniqueid.length; loop++) {
         ...
         fieldMap.populateContainer(ResourceField.class, resource, id, new byte[][] { data }, rscVarData);
         resource.enableEvents();

         processHyperlinkData(resource,
               rscVarData.getByteArray(id, fieldMap.getVarDataKey(ResourceField.HYPERLINK_DATA)));
         resource.setID(Integer.valueOf(MPPUtility.getInt(data, 4)));

         resource.setOutlineCode1(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE1_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode2(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE2_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode3(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE3_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode4(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE4_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode5(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE5_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode6(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE6_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode7(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE7_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode8(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE8_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode9(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE9_INDEX), OUTLINECODE_DATA));
         resource.setOutlineCode10(m_outlineCodeVarData.getUnicodeString(
               (Integer) resource.getCachedValue(ResourceField.OUTLINE_CODE10_INDEX), OUTLINECODE_DATA));
         ...
         af.process(resource.getAvailability(),
               rscVarData.getByteArray(id, fieldMap.getVarDataKey(ResourceField.AVAILABILITY_DATA)));
   ...
   private void processAssignmentData() throws IOException {
      ...
      Var2Data assnVarData = new Var2Data(assnVarMeta,
            new DocumentInputStream(((DocumentEntry) assnDir.getEntry("Var2Data"))));

      FixedMeta assnFixedMeta = new FixedMeta(new DocumentInputStream(((DocumentEntry) assnDir.getEntry("FixedMeta"))),
            34);
      ...
      factory.process(m_file, fieldMap, null, m_reader.getUseRawTimephasedData(), m_reader.getPreserveNoteFormatting(),
            assnVarMeta, assnVarData, assnFixedMeta, assnFixedData, null, assnFixedMeta.getAdjustedItemCount());
   }
   ...
   private void processViewData() throws IOException {
      ...
      Var2Data viewVarData = new Var2Data(viewVarMeta,
            new DocumentInputStream(((DocumentEntry) dir.getEntry("Var2Data"))));
   ...
   private void processTableData() throws IOException {
      ...
      // FixedMeta fixedMeta = new FixedMeta(getEncryptableInputStream(dir,
      // "FixedMeta"), 9);
   ...
   private void processFilterData() throws IOException {
      ...
      // FixedMeta fixedMeta = new FixedMeta(new DocumentInputStream(((DocumentEntry)
      // dir.getEntry("FixedMeta"))), 9);
      // FixedData fixedData = new FixedData(fixedMeta, getEncryptableInputStream(dir,
      // "FixedData"));
      InputStream stream = m_inputStreamFactory.getInstance(dir, "FixedData");
      int blockSize = stream.available() % 115 == 0 ? 115 : 110;
   ...
   /**
    * This method is called to try to catch any invalid tasks that may have sneaked
    * past all our other checks. This is done by validating the tasks by task ID.
    */
   private void postProcessTasks() {
      ...
      if (allTasks.size() > 1) {
         ...
         for (int i = 0; i < allTasks.size(); i++) {
            ...
            // In Project the tasks IDs are always contiguous so we can spot invalid tasks
            // by making sure all
            // IDs are represented.
   ...
   // {
   // System.out.println (spec[loop][0] + ": "+ ByteArrayHelper.hexdump(data,
   // spec[loop][0], spec[loop][1], false));
   // }
   // System.out.println ();
   // }
   ...
   /**
    * Deleted and null tasks have their ID and UniqueID attributes at fixed
    * offsets.
    */
